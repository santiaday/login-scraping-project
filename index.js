...
const githubToken = [
  "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",
  "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",
  "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",
];
...
const octokit = new Octokit({
  ...
  await browser.task(async ({ page, data: pageUrl }) => {
    try {
      ...
      let repoUrl =
        "https://raw.githubusercontent.com/santiaday/dlWebflow/main/" +
        fullPath;

      let chosenToken = githubToken[Math.floor(Math.random() * 3)];
      const options = {
        headers: {
          Authorization: `token ${chosenToken}`,
        },
      };

      let existingContent = "";

      await fetch(repoUrl, options)
